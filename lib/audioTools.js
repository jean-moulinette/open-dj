		...
		audioExist : function(data, sockets, force){
			...
			fs.readdir('./downloaded', function(err, files){
				...
				if(result){
					...
					forkedProcess = child.fork('./lib/audioTools.js', ['playSound', data.id, data.title]);
 ...
 *
 *	@param: {[string]}	- songName - Le nom du fichier mp3 à lancer sur la machine
 */
function playSound(songName){

	//Recuperation du nom de fichier à lancer
	var fileToPlay = songName;
