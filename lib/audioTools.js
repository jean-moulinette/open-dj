...
//Cas de l'execution par le process main
if(typeof process.argv[2] == 'undefined'){
	...
	if(process.argv[2] === 'searchYoutube'){
		...
		//Transormation des accents en ascii
		var removeDiacritics = require('diacritics').remove;

		process.argv[3] = removeDiacritics(process.argv[3]);
	...
	if(process.argv[2] === 'playSound'){
		...
		var playingProcess = child.exec('cd downloaded && mplayer -- ' + fileToPlay + ' > log_play.log');
...
function searchYoutube(data){
	...
	var urlPath = '/results?search_query='+data;
 ...
 */
exports.audioExist = function(data, sockets, force){
	...
	fs.readdir('./downloaded', function(err, files){
		...
		if(result){
			...
			forkedProcess = child.fork('./lib/audioTools.js', ['playSound', data.id+'.mp3', data.title]);

		}else{
			//Fork du process main pour télécharger le fichier mp3
			forkedProcess = child.fork('./lib/audioTools.js', ['downloadMp3', data.id, data.title]);
		}
		...
		forkedProcess.on('message', function(data){
			...
			if(data.task === 'update-music'){
				...
				playingStatus.on = false;
				playingStatus.title = 'En attente d\'une demande de lecture';

				//Mise à jour de l'afficheur de titre chez les clients
				sockets.emit('update-current-music', 'En attente d\'une demande de lecture');
		...
		forkedProcess.on('close', function(code, signal){
			...
			console.log('Le child de lecture viens de mourir\n');
...
exports.searchVideo = function(data, socket){
	...
	requestWorker.on('message', function(retour){
		...
		//Si l'espion de recherche est à null, on va mle mettre à jour
 ...
 *	@return: void
 */
exports.modifyVolume = function(choice){
...
exports.downloadCurrentSong = function(){

	//Trouver un moyen de délivrer le fichier .mp3 au socket unique
