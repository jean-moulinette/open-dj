	...
	init : function(io, audioTools){
		...
		io.sockets.on('connection', function(socket){
			...
			socket.on('yt-search', function(data){
				...
				console.log('\n Un client effectue une recherche : '+data+'. \n');
			...
			socket.on('playlist', function(data){
				...
				if(action === 'add'){
					...
					console.log('Ajout d\'une musique dans la playlist.');

					playingStatus.playlist.push({
						id:data.id,
						title:data.title
					});

					//Ecriture de l'action pour la méthode de reception coté client
					data.action = 'update-playlist'

					//Envoi d'un evenement refresh playlist à tous les sockets
					io.sockets.emit('playlist', playingStatus.playlist);
